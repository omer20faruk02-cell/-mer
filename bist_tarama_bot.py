...
Bu script şunu yapar (DİPTEN AL mantığı):
...
2. Senin belirlediğin kriterlere göre filtreler:
   ...
   (Günlük %5+ yükseliş şartı KALDIRILDI — bu bot artık bugün zaten
   fırlamış hisseleri değil, desteğe yakın/destekten tepki alan
   hisseleri arıyor; bu ikisi mantık olarak çelişiyordu.)
...
def screen_stock(ticker: str, df: pd.DataFrame):
    ...
    change_pct = (last_close - prev_close) / prev_close * 100  # sadece bilgi/gösterim amaçlı
    ...
    # Günlük yükseliş şartı YOK — dipten al mantığında hisse bugün düz,
    # hatta hafif kırmızı olabilir; önemli olan hacimli ilgi + destek kurulumu.
    passes = (
...
def analyze_stock(df: pd.DataFrame):
    ...
    bir AL kurulumu varsa sonuç döner, yoksa (None, sebep) döner.
    'sebep' değeri diagnostik özet için kullanılır."""
    close = df["Close"]
    ...
    if raw_potential_pct <= 0:
        return None, "direnc_altinda_yer_yok"  # fiyat zaten direncin üstünde/eşiğinde
    ...
    if not (near_support or bounced_from_support):
        return None, "destege_yakin_degil"  # sadece destek yakınında veya tepki alan kurulumlar

    if expected_rise_pct < MIN_EXPECTED_RISE_PCT:
        return None, "beklenen_yukselis_dusuk"  # minimum %MIN_EXPECTED_RISE_PCT şartı
...
def build_html_report(results: list[dict], stats: dict) -> str:
...
<style>
  ...
  .diag {{ margin-top:20px; font-size:0.85rem; color:#c8c8d0; }}
  .diag summary {{ cursor:pointer; color:#9a9aa5; }}
  .diag table {{ margin-top:8px; }}
  .empty {{ color:#9a9aa5; }}
...
<body>
  ...
  <div class="criteria">Tarama: göreceli hacim &gt;{MIN_REL_VOLUME}, hacim &gt;{MIN_VOLUME_TL:,.0f} TL ·
    AL şartı: beklenen yükseliş &gt;=%{MIN_EXPECTED_RISE_PCT} ve desteğe yakın/destekten tepki
    — {len(results)} hisse eşleşti</div>
  ...
  <details class="diag">
    <summary>🔍 Tarama özeti (neden az/çok hisse çıktı)</summary>
    <table>
      <tr><td>Taranan toplam hisse</td><td>{stats['toplam']}</td></tr>
      <tr><td>Veri çekilemedi</td><td>{stats['veri_yok']}</td></tr>
      <tr><td>Hacim filtresini geçemedi (rel. hacim/ciro)</td><td>{stats['hacim_filtresi_gecmedi']}</td></tr>
      <tr><td>Fiyat dirence çok yakın/üstünde (yer yok)</td><td>{stats['direnc_altinda_yer_yok']}</td></tr>
      <tr><td>Momentum negatif</td><td>{stats['momentum_negatif']}</td></tr>
      <tr><td>Desteğe yakın değil / tepki yok</td><td>{stats['destege_yakin_degil']}</td></tr>
      <tr><td>Beklenen yükseliş %{MIN_EXPECTED_RISE_PCT}'in altında</td><td>{stats['beklenen_yukselis_dusuk']}</td></tr>
      <tr><td><b>AL kriterlerine uyan</b></td><td><b>{stats['basarili']}</b></td></tr>
    </table>
  </details>
...
def main():
    ...
    # Diagnostik sayaçlar — hangi aşamada kaç hisse elendiğini görmek için
    stats = {
        "toplam": len(tickers),
        "veri_yok": 0,
        "hacim_filtresi_gecmedi": 0,
        "direnc_altinda_yer_yok": 0,
        "momentum_negatif": 0,
        "destege_yakin_degil": 0,
        "beklenen_yukselis_dusuk": 0,
        "basarili": 0,
    }
    ...
    for ticker in tickers:
        ...
        if screened is None:
            stats["hacim_filtresi_gecmedi"] += 1
            continue
        analysis, reason = analyze_stock(df)
        stats[reason] += 1
        if analysis is None:
    ...
    print("Tarama özeti:", stats)

    # Web sitesi (GitHub Pages) için HTML üret
    os.makedirs("public", exist_ok=True)
    html = build_html_report(results, stats)
